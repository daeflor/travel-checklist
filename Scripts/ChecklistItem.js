function ChecklistItem(rowId, itemName, needed, inCart)
{
    var Elements = {
    ...
    {
        console.log("Create a column with value: " + value);
        ...
        //TODO might be better to create both buttons and then just show or hide them on demand

        var button = CreateNewElement(
            'button', 
            [ ['class','btn'], ['type','button'] ], 
            CreateNewElement('i', [['class',iconClass]])
        ); //TODO would be nice to be able to pass options to CreateButtonWithIcon

        //Elements.buttons.push(button);
        Elements.wrapper.appendChild(CreateNewElement('div', [ ['class','col'] ], button));
    }
    ...
    function ToggleColumnValue(index)
    {
        //if (Elements.buttons[index].value
    }

    function UpdateColumnIcon()
    {
